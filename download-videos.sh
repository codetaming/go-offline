#!/usr/bin/env bash
brew install youtube-dl
mkdir -p  "GopherCon"
cd "GopherCon"
mkdir -p  "2018"
cd "2018"
youtube-dl https://www.youtube.com/watch?v=zPd0Cxzsslk&list=PL2ntRZ1ySWBdatAqf-2_125H4sGzaWngM
cd ..
mkdir -p  "2017"
cd "2017"
youtube-dl https://www.youtube.com/watch?v=ha8gdZ27wMo&list=PL2ntRZ1ySWBdD9bru6IR-_WXUgJqvrtx9
cd ..
mkdir -p  "2016"
cd "2016"
youtube-dl https://www.youtube.com/watch?v=KINIAgRpkDA&list=PL2ntRZ1ySWBdliXelGAItjzTMxy2WQh0P
cd ..
mkdir -p  "2015"
cd "2015"
youtube-dl https://www.youtube.com/watch?v=3EW1hZ8DVyw&list=PL2ntRZ1ySWBf-_z-gHCOR2N156Nw930Hm
cd ../..
mkdir -p  "London Gophers"
cd "London Gophers"
mkdir -p  "August 2018"
cd "August 2018"
youtube-dl https://www.youtube.com/watch?v=7cEp98y6WCs&list=PL8QGElREVyDA2iDrPNeCe8B1u7li5S6ep
cd ..
mkdir -p  "June 2018"
cd "June 2018"
youtube-dl https://www.youtube.com/watch?v=0XW6wI2FnPA&list=PL8QGElREVyDB1y7q-tarloT2PHhOP-ZIK
cd ..
mkdir -p  "May 2018"
cd "May 2018"
youtube-dl https://www.youtube.com/watch?v=eTjL3grAYAM&list=PL8QGElREVyDBLA4OXGvNELXAtSznHK_fw
cd ..
mkdir -p  "March 2018"
cd "March 2018"
youtube-dl https://www.youtube.com/watch?v=kWAxBhsEayk&list=PL8QGElREVyDDDtiVmCjh_zBkY4KnZmHxb
cd ..
mkdir -p  "January 2018"
cd "January 2018"
youtube-dl https://www.youtube.com/watch?v=JqnqDeRZxas&list=PL8QGElREVyDDimqKDNDRpB9MPTq_dPIc5
cd ..
mkdir -p  "November 2017"
cd "November 2017"
youtube-dl https://www.youtube.com/watch?v=BBiIr19JOo4&list=PL8QGElREVyDCMDKnpmkvPzyE2DJRaRISj
cd ../..
mkdir -p  "GopherCon UK"
cd "GopherCon UK"
mkdir -p  "2018"
cd "2018"
youtube-dl https://www.youtube.com/watch?v=2mgKDqD5Ga8&list=PLDWZ5uzn69ewsMyuGjVsAnpQIjyud1Cv9
cd ..
mkdir -p  "2017"
cd "2017"
youtube-dl https://www.youtube.com/watch?v=5UG57xQL_RE&list=PLDWZ5uzn69eyM81omhIZLzvRhTOXvpeX9
cd ..
mkdir -p  "2016"
cd "2016"
youtube-dl https://www.youtube.com/watch?v=LpWEDFT3iLU&list=PLDWZ5uzn69eyh791ZTkEA9OaTxVpGY8_g
cd ..
mkdir -p  "2015"
cd "2015"
youtube-dl https://www.youtube.com/watch?v=CdhucJShJU8&list=PLDWZ5uzn69ezRJYeWxYNRMYebvf8DerHd
cd ../..
mkdir "Learn to Code"
cd "Learn to Code"
mkdir "Golang 2019"
cd "Golang 2019"
youtube-dl https://www.youtube.com/playlist?list=PLSak_q1UXfPqKzZiiNcE5sFIXD7QrTyig
cd ..
mkdir "Golang Web Course"
cd "Golang Web Course"
youtube-dl https://www.youtube.com/watch?v=uMhpCDZAebE&list=PLSak_q1UXfPrba68q2afhFq4aBAPVSDbj
cd ../..
mkdir "Other"
cd "Other"
youtube-dl https://www.youtube.com/watch?v=5gJFQnEl5ZQ
youtube-dl https://www.youtube.com/watch?v=dVnMLtdJzn4
youtube-dl https://www.youtube.com/watch?v=kIKwPNKXaLU
cd ..
mkdir -p  "GopherCon Russia"
cd "GopherCon Russia"
mkdir -p  "2018"
cd "2018"
youtube-dl https://www.youtube.com/watch?v=ZCB-g2B4Y5A&list=PLJTW0ZQ22rrH8rWZBs7qMonKZxW4osXE2